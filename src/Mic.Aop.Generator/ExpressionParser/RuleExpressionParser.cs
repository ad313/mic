...
namespace Mic.Aop.Generator.ExpressionParser
{
    public class RuleExpressionParser
    {
        private readonly ReSettings _reSettings;

        private static ConcurrentDictionary<string, Func<object[], bool>> _memoryCache =
        ...
        public Expression Parse(string expression, ParameterExpression[] parameters, Type returnType)
        {
            ...
            return new System.Linq.Dynamic.Core.Parser.ExpressionParser(parameters, expression, new object[] { }, config).Parse(returnType);

        }

        public Func<object[], bool> Compile(string expression, RuleParameter[] ruleParams)
        {
            ...
            var cacheKey = GetCacheKey(expression, ruleParams, typeof(bool));
            return _memoryCache.GetOrAdd(cacheKey, (d) =>
            {
                ...
                if (rtype == null)
                {
                    e = Expression.Convert(e, typeof(bool));
                }
                var expressionBody = new List<Expression>() { e };
                var wrappedExpression = WrapExpression<bool>(expressionBody, parameterExpressions, new ParameterExpression[] { });
